"""
결과 조회 api추가 // DB 버전 (더미데이터로 테스트용, MySQL)(코랩, 코랩 - 로컬DB 불가로 파기, 참고용)
"""

!pip install fastapi uvicorn pyngrok "opencv-python-headless<4.3" python-multipart torch torchvision torchaudio nest_asyncio sqlalchemy pymysql
...
from urllib.parse import quote_plus # password 인코딩용

# 환경 변수 설정
os.environ["NGROK_AUTH_TOKEN"] = "2jGzT5KrMCTO5lJZq68sIhvwo2N_41kScxnYWbstYACqQqjHS"
...
# ====================================
# MySQL 설정 (외부 DB - PlanetScale 또는 AWS RDS)
# username, password, host, port, dbname은 실제 환경에 맞게 수정
# 아래 두개는 유료인 관계로 추후 고려
# PlanetScale 예시:
# DATABASE_URL = "mysql+pymysql://<username>:<password>@<host>/<dbname>?ssl_ca=/etc/ssl/cert.pem"
#
# AWS RDS 예시:
# DATABASE_URL = "mysql+pymysql://admin:<password>@mydb.c9akciq32.us-east-1.rds.amazonaws.com:3306/testdb"
# ====================================
db_user = "fastapi"
db_password = "Fastapi123@@"
db_host = "127.0.0.1"   # RDS나 외부 DB 호스트로 변경 가능
db_port = 3306
...
-DB는 MySQL 외부 DB 사용 (PlanetScale 또는 AWS RDS)
  -> PlanetScale: 학습/테스트에 적합 (무료)
  -> AWS RDS: 실서비스 배포에 적합
- Colab 테스트용: ngrok + public_url 사용
- 실제 서버 배포: ngrok 제거, 서버 도메인/IP + 포트 -> AI_URL 사용
- DB 저장 (memory_storage 대신 외부 DB에 저장)
